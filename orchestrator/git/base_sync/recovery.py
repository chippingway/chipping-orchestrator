...
Beside that route sits a second one, and it is DORMANT: no production selector
reaches `_recover_vouched_replay_context`, so every interrupted rebase the
refresh finds is still recovered on the anchor and the counts above. What it
is built for is the attempt the counts cannot read. A rebase REPLAYS the
branch, so the canonical pre-push recovery is behind its own publication and
parks as divergence, and the rebase may have been carrying a human's verdict
onto the commit it produced. ``transfers`` answers the second half off the
pinned comment -- how far the transfer's own writes got -- and ``attempts``
the first, off the pair of heads the attempt recorded. A rewrite the grant
never reached is given re-derived evidence, so the replay is decided on the
transfer the dead tick would have asked for rather than measured past the same
ceiling and adjudicated a second time with a pull request open over the work.

Every road out of that route ends in a notice, an audit event, and the anchor
...
The counts are a fallback there for one state alone -- a comment carrying no
record of a replay at all -- and the window between git returning and the
write that names one has a road of its own, where the head is proved by what
it contributes rather than by an id nobody wrote down. Every state neither of
...
def _recover_vouched_replay_context(
    ...
    The three steps of the route above, in the same order, each answered by
    ...
    No production selector calls this. The refresh enters the route above and
