...
The first five are the running route's. Every answer past them is reached only
from the vouched-replay route in ``recovery``, which is DORMANT: no production
selector enters it yet, so those parks are the closed set that route will
resolve into once one does, and nothing a live tick does can reach them.
