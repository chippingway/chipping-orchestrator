...
def _sync_unreadable_pr_worktree(request: _AutoRebaseRequest) -> None:
    ...
    stage left is still left alone, a park this refresh left is still waiting
    on a reply, and a pull request that ended or would not read is still
    answered as it is there. Only the step past them differs. There is no lag
    to route on and no head worth comparing, so nothing is fetched and nothing
    is classified: the recovery takes its fail-closed abort directly.
    """
    ...
    if eligibility._open_auto_rebase_pr(context) is None:
        return
    recovery._answers_an_unreadable_checkout(
