...
class _PendingRewrite:
    ...
    the branch: taken from the issue as it reads after a crash they would
    compare today with today, so a relabel or a repoint made while the process
    was down would pass as the dead tick's own terms. The head is what the
    rebase produced, so it cannot exist until git hands it back -- and it is
    what says the checkout in front of a recovery is that attempt's own work.
    ...
    Which is why the terms are still worth having there -- what vouches for it
    is asked over the dead tick's own publication rather than over whatever
    the issue says on the tick that finds it.
    ...
    existed, or one whose anchor is all that was ever pinned. Declared is the
    in-flight window above. Damaged is a comment that claims the record and
    cannot show it -- a member taken out, a head that is not a whole git
    ...
    def answers_for(
        ...
        What a road has to ask before it finishes anything, because finishing
        is not silent: a notice goes to the pull request this tick holds, an
        audit event is filed under the stage this tick reads, and the anchor
        that would bring the tick back is dropped. A repoint or a relabel made
        while the process was down would have all three attributed to a
        publication the interrupted attempt was never made for -- and the
        record is the only thing that can say so.

        Answered on the terms alone, with no exception for the stage a finish
        moves to. That relabel IS a state a recovery has to recognize -- it is
        the last thing a finish does before the write that clears this record
        -- but the label on its own cannot say whether it was this route's
        step or somebody else's, and an issue moved to `validating` after a
        crash the push never survived looks identical.
        """
        return (
            self.is_recorded
