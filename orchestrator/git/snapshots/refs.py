...
The `mirrors.py` owner handles repository-qualified local names and ref-store
probes. This owner keeps the lock across fetch and verification and orders mirror
removal ahead of remote deletion.
...
def prove_snapshot_ref(
    ...
    if not namespace.is_snapshot_ref(ref):
        ...
        return SnapshotOutcome.REFUSED
    mirror = _mirrors.local_snapshot_ref(spec, ref)
    ...
    with locks._target_root_lock(spec.target_root):
        ...
        if fetched.returncode != 0:
            ...
            return SnapshotOutcome.REFUSED
        resolved = _mirrors._local_ref_sha(worktree, mirror)
    if resolved == sha:
...
def delete_snapshot_ref(
    ...
    **This host's copy goes first, and the remote is not touched until it has
    ...
    if observed not in _RECLAIMABLE:
        return observed
    if not _mirrors._mirror_dropped(spec, worktree, ref):
        return SnapshotOutcome.REFUSED
