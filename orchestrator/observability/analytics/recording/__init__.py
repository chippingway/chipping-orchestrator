...
record costs to produce: the sink append and the four producer-facing
...
This initializer re-exports the narrow public surface (``__all__``): the seven
recorders a producer calls -- the envelope builder and the append beneath
them, plus one each for a stage entered, a stage evaluated, a human park
recorded, a repo's skill catalog scanned, and a tracked agent run finished. Each
is bound here once, at import, to the object its owner defines rather than a
wrapper around it: the append and the four direct recorders report ``events``,
the sequenced one ``agent_exit``, and the envelope ``sink``, which ``events``
republishes because that is the import site a producer already names.
Everything else -- the typed requests, the parse steps, the JSONL append's
arguments -- is reached on its owner, so this package carries no private
re-exports.
...
record_park_awaiting_human = _events.record_park_awaiting_human
