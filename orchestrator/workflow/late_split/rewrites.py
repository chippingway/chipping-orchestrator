...
One field of that write is not about the move at all. Which reading PROVED
the push had landed -- the leased force-push that moved the remote, or the
leased no-op that found it standing on the rewritten commit already -- is the
one fact nothing later could re-derive, since the receipt looks identical
either way, and the record of the move goes to the sinks behind the write that
makes it. So the proof is kept on the comment until that record is out and
dropped by the write behind it, which leaves a process lost in between
something to report from rather than a verdict that moved with nothing
anywhere saying so. It stands outside the group a reader is held to whole,
because a transfer is settled whether or not it has been reported.
