...
refuses rather than recording a field the gate would read. Which is why the
writers, the readers, and the presence question sit in ONE owner: the shape a
field is written in and the shape it is refused in are the same decision, and
split across modules a writer would be free to put down what the reader beside
it cannot vouch for.
"""
