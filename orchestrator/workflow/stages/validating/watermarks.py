...
walked past as well: it is an operator control, read only while its run-limit
park stands and never feedback for anybody, and the grant that answers it can
leave it unread above the reply its park interrupted -- where a walk stopping
on it would seed the watermark below it and in_review would route the issue to
`fixing` over a command already handled.
