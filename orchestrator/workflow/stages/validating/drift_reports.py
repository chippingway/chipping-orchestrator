...
def _withholds_the_stranded(state: PinnedState, run: _models._DevFixRun) -> bool:
    ...
    A run that COMMITTED is not this question, whatever the branch carries
    under it. It owes a report of its own before anything it made may be
    published, and the recording ahead of the gate is what holds it to that
    -- so a reply with none parks there, asking the human for the report both
    commits are short of, rather than being posted as an acknowledgement of
    an edit this run answered in code.
    """
    stranded = run.published_head and run.after_sha == run.before_sha
    if run.handed is None or not stranded:
        return False
    if _reports(run.agent_result):
        return False
    _owes_the_undescribed(state)
