...
def _waives_an_incomplete_run(
    ...
    *,
    ...
    nothing. Written before the gate, since a recovery after a process that
    died in it reads the waiver off the pinned comment, and one only held in
    memory would park that commit as a report somebody lost.

    A run that DID complete retires it instead, in memory: the report the
    seam records next describes the branch it leaves, commits an earlier
    unfinished run made included, so `_holds_an_unfinished_run` has nothing to
    hold -- and the write that records that report, or parks the run for the
    one it lacks, carries the retirement with it. Written apart, a process
    dying between the two would leave the new report beside a waiver it
    retired, and a recovery would hold the report as an older run's.
    """
    if not (stranded or work.agent_result.invoked):
        return
    outcome = _report_outcomes._report_outcome_of_run(work.agent_result)
    if not (stranded or outcome in _report_delivery.INCOMPLETE_RUNS):
        if state.get(_state._INCOMPLETE_RUN_SHA):
            state.set(_state._INCOMPLETE_RUN_SHA, None)
        return
    head = _verification_probes._head_sha(work.worktree) or None
    if state.get(_state._INCOMPLETE_RUN_SHA) != head:
        state.set(_state._INCOMPLETE_RUN_SHA, head)
...
def _holds_unreported_work(
    ...
    if settled is not None:
        return _report_handoff._holds_a_moved_settlement(gh, issue, state, settled)
    _report_delivery.parks_an_undeliverable_report(
        gh, issue, state, _UNRECOVERED_PARK.format(
            mentions=config.HITL_MENTIONS,
        ),
    )
    return True
