...
consumes nothing: the developer never read the batch, and a watermark moved
for them is a human reply answered by nobody. A run that finished consumes it
whatever it came back with, which is what keeps the validating -> in_review
handoff from replaying the same reply as fresh PR feedback. Nothing is lost by
waiting for the outcome, because the settlement is in memory either way: every
caller that returns on a pause or an interruption returns WITHOUT writing
pinned state, so a bump taken ahead of the run would be dropped by that return
anyway -- and a run that crashed outright takes the whole tick's writes with
it.
"""
...
def _resume_developer_on_human_reply(
    ...
    *,
    ...
    settlement advances the issue watermark alone: a PR comment sitting below
    the reply consumed here was never in this prompt, and a maximum taken
    across both surfaces would skip it for good.

    `batch` is the caller's own frozen read, and it carries the pinned state
    that read was bounded by rather than taking one beside it: `validating`
    hands over the very batch its park-reason decisions were made from, and
    what the run consumes is settled onto the same reading it came out of
    rather than onto some other reading of the same issue. What is IN it --
    the trust filter, the orchestrator's own comments, a forged marker, and
    the two parks whose command belongs to nobody here -- is `resume_batch`'s
    to decide; this owner honors `reserved` by resuming nothing and consuming
    nothing, because a watermark is one number and the resume is not the last
    thing to move it: the park its run takes would stamp the thread read to a
    notice above the command and take it for good.

    `last_action_comment_id` is settled once the run is back, from that frozen
    batch, and only for an outcome that counts it as delivered. Without the
    settlement a successful resume during implementing or validating leaves
    the watermark at the prior park id, so the validating -> in_review handoff
    treats the just-consumed reply as fresh PR feedback and re-resumes the dev
    on input it has already handled. With it taken on an outcome the developer
    never read -- a launch the run circuit refused, a shutdown kill, a live
    pause -- the reply would be marked answered by a run that never happened.
    A timeout or an empty result is neither of those: the prompt reached the
    agent, so the batch is consumed and the failure is surfaced through the
    timeout / dirty / question parks instead.
    """
