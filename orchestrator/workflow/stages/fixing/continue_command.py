...
A park whose issue-action boundary sits BELOW the ids its own bookmarks name
is where that shows. The replay is the tick that quotes those recorded
comments again, so it is the tick that owes each reader its own half of them:
attributed wholesale, an issue-thread reply settles the pull request's cursor
and leaves the boundary where it was, and the relabel out of `fixing` after it
pays a second developer to deliver the very comment this retry just answered.
