...
def _places_the_round_in_hand(state) -> bool:
    ...
    came next, which is a round it says nothing about. It is also the only
    thing that catches an anchorless manual move back to `workflow:fixing`,
    since the settlement cleared both route anchors and dropped the
    transaction, leaving the comment looking exactly like a round that closed.
