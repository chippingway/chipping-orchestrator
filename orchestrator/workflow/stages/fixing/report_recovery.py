...
"""A report this issue recorded and never bound, and the round it closes.
...
input the run consumed rides the same record, and until something applies it a
scan reading past it counts that feedback as unread -- pays a second developer
to answer it, and replaces the first report with the second. That is why this
road belongs AHEAD of a scan rather than behind one, and why every helper here
is written to be asked before anything else a tick would do.
...
road and the live one answer the identical comment the same way. The
...
_UNPUBLISHABLE_PARK = (
    ...
    "carrying uncommitted changes, and no road here will publish a report over "
    "a checkout in that state. Nothing was published, because doing it on a "
    "guess would hand a reviewer a description of work that may not be there "
    "-- and that recorded report is RELEASED here rather than left waiting, "
    "since restoring or cleaning the checkout would otherwise be enough on its "
    "own to publish it and send the issue back to review, which is the one "
    "thing this notice is asking a human to decide. The branch, the pull "
    "request, and the debt are untouched: this issue still owes its pull "
    "request a report, and the review stays held until one is there. Clean up "
    "or restore the checkout if you want that commit published, then reply: "
    "the orchestrator resumes the developer, and the report that session "
    "writes is the one that gets published. It needs no new commit to deliver "
    "it."
)
...
def _finishes_a_settled_round(ctx: _models._FixingContext) -> bool:
    ...
    Asked from both sides of the binding below, because the answer is the same
    question each time: ahead of everything, for a settlement some other write
    landed, and behind that binding for the one it landed itself. A settlement
    is not licence to relabel -- the mark it raised is, and only where this
    owner can still place it.
    ...
    round is over and the issue is still sitting on `workflow:fixing`. Carried
    on past, a scan reads whatever landed since under a route the settlement
    has just closed, and answers an in_review batch as a validating one: an
    ordinary `ACK:` is refused or parked instead of returning the pull request
    to review.
...
def _recovers_an_unbound_delivery(ctx: _models._FixingContext) -> bool:
    ...
    True is a tick this call ended. Presence is asked before meaning, and that
    order is what keeps a damaged record from being read as an issue with
    nothing outstanding. The debt every road behind this reads is CLAIMED by
    the key alone, so a record a hand edit truncated is a debt nothing here
    could describe -- and read as an absence it would fall straight through to
    a scan, whose watermarks that same record was holding back, and pay a
    second developer to answer the feedback the first one already answered.
    Parked once instead, with the record untouched for whoever repairs or
    abandons it.

    A reading nobody could TAKE is the one answer that ends the tick saying
    nothing at all -- not a publication, not a release, not a notice, not even
    a write. It says nothing about the branch, so it may buy nothing; and a
    road behind this one would make it buy the notice, since the record's own
    pairs cover the batch and a scan therefore finds nothing to act on.
...
def _binds_the_recovered_report(
    ctx: _models._FixingContext, published: str,
) -> bool:
    """Put the recovered report on its publication; True is a tick ended here.

    Settled, the round the record froze is closed and the issue goes back to
    `workflow:validating`, which is what the live road would have done and what
    keeps a scan behind this from reading the issue under a route the
    settlement has just cleared. Bound but unposted, nothing is relabelled: the
    transaction is the reconciliation's to finish, and the bookmarks it replays
    from have to outlive this tick.
    ...
    False is the delivery a binding refused without consuming -- a pull request
    standing somewhere else, a thread that has ended, a comment too full.
    Nothing is discarded there, the tick carries on, and a later one asks
    again; stopping instead would hold the roads that answer those in front of
    a condition only a human clears.

    A pull request reading this poll could not TAKE is the exception, for the
    reason a checkout nobody could read is: it says nothing about the branch,
    the description or the head, so it may buy nothing -- and a road behind
    this would make it buy a notice. It buys no WRITE either, which is why it
    is answered ahead of everything below: the attempt staged nothing, so the
    comment such a write would leave is the one already on the issue, and
...
def _releases_an_unpublishable_report(
    ...
    Taken on a checkout the caller has PROVED refuses for good, and a definite
    refusal is one no later poll answers differently -- so leaving it to the
    next tick is leaving the issue to find no feedback, no publishable checkout
    and an owed report every poll, and quietly do nothing at all with any of
    them.

    A worktree that is GONE is the first of the two: nothing to republish and
    nothing to prove, since the commit the report describes is either already
    on the pull request or went with the checkout and no reading left here can
    say which. A tree this host PROVED dirty is the second, and it is the one
    the stranded republishing bounce cannot help with either: that road refuses
    a dirty checkout exactly as this one does, so the two decline together and
    nothing moves until somebody cleans it.

    A status nobody could read is neither, which is why the proof is the
    caller's: a later poll may read it, and a head ahead of the pull request is
    a checkout a republishing road still sends from, with the report going out
    on the push it makes.
    ...
    not. Left on the comment, the record is one the very next tick can bind the
    moment the checkout comes back -- so a human who merely restores or cleans
    it, without the reply this notice asks for, publishes the report nothing
    could place and sends the issue to review on it. That is the decision the
    notice exists to put in front of them, and a condition clearing itself is
    no answer to it. What the debt keeps is everything that holds: the review
    stays held, the roads that read an owed report still read one, and the
...
def _published_checkout(ctx: _models._FixingContext) -> str | None:
    ...
    through to the road that re-proves the remote and republishes the commit
    rather than parking over a copy nobody refreshed.
    """
