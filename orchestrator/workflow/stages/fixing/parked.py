...
The silent recovery is third and is the narrowest of the four. It only fires
on the validating route, because that is the route whose transient park
(`push_failed` / `agent_timeout`) landed under `fixing` without any human
involvement -- the CHANGES_REQUESTED branch relabels before it spawns the dev
...
def _recovers_without_a_human(state, park_reason, *, owes: bool) -> bool:
    ...
    default below holds this issue on a park nothing can clear -- no later
    push is ever attempted, and the only ways out are a human comment or an
    `/orchestrator continue`, both of which pay a second developer to answer a
    batch the first one already answered.
    """
