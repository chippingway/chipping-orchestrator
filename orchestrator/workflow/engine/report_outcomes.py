...
`_parse_report_outcome` is the message half alone, `_finished_on_a_report` is
that same reading asked as a yes/no by a stage whose other replies are ordinary
answers rather than a contract broken, and `_reached_for_the_contract` is the
one reply such a stage may not treat as an ordinary answer either.
