...
def _reconciles_pending_report(
    ...
    A record this owner WOULD park, on an issue another route has already
    parked, is False too, and that one is deliberate rather than incidental.
    The route that answers a foreign park is the stage handler behind this
    guard, so holding in front of it would leave both parks standing for the
    life of the issue with neither the damage nor the recovery announced.
    `_parks_the_damage` below carries the whole of that reasoning.
...
def _parks_the_damage(
    ...
    """Answer a report record nobody can read, and say whether the tick stops.
    ...
    finds this owner's own park already standing is held silently, and the
    moment the record reads again the ordinary reconciliation resumes.

    A park ANOTHER route already took is left exactly as it is, and no park of
    this owner's is taken beside it. The flags are single: writing ours over an
    ...
    That case STANDS DOWN rather than holding, and the difference is the whole
    of whether either park is ever answered. What clears a foreign park is the
    stage handler behind this guard -- the awaiting-human branch that reads the
    human's reply, recovers a transient park, or times one out. Held here, that
    branch never runs, so the foreign park never clears, so `awaiting_human`
    stays set and this holds again: both parks stand for the life of the issue
    and no notice is ever posted for either the damage or the recovery. Stood
    down, the handler runs and answers what it is waiting on, and the tick
    after it clears is the one that takes this owner's park and announces the
    damage.

    Nothing is lost by waiting for that tick. The issue is already held
    awaiting a human, which is what this park would have asked for; the record
    is untouched and still unreadable when that park clears; and the damage is
    announced then rather than never.
    """
    ...
    if state.get(_AWAITING_HUMAN):
        log.warning(
            ...
            "(%s) while another route's park stands (%r); standing down so "
            "that park can be answered, and announcing the damage once it is",
