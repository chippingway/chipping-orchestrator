...
def _grant_runs(
    ...
    launch was on is put back (`run_limit_state._restore_displaced`), so a
    resume refused on a reply is resumed on that same reply from its own frozen
    batch -- not quoted by an ordinary spawn that records nothing, or handed to
    a reviewer on `validating` when it was written for the developer.
    """
...
def _answered_through(
    ...
    Unless the read BEGINS below a notice of ours. The bounded park leaves a
    reply it could not walk past under its own notice, and that reply is no
    answer to the park: it is the input a refused resume was handed and no
    agent read. A watermark is one number, so the command above it cannot be
    consumed without it: the walk starts at the mark instead, crosses our own
    comments, and stops at that reply. The command is left unread with its
    receipt on the thread, and every later reader cuts a bare one out.
