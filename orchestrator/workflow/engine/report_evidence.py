...
for the callers that made the publication or settle it -- and beside the ISSUE
that re-read produced, since a settlement stamps its handoff with the label that
issue is carrying and two fetches would be two answers to one question. And
`refuses_for_good` says, posting nothing, whether an owed transaction can ever
settle as it stands.
"""
