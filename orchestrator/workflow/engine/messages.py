...
def _refuse_parked_continue(
    ...
    from it rather than from the thread's tip: guidance can land between the
    caller's read and this post, and the tip would mark it answered by a
    refusal that never considered it. Past the refusal itself the
    `park_watermarks` walk carries the mark through our own identified
    comments and no further. A caller whose own settlement consumes the batch
    passes nothing and the walk is the whole of what happens.
    """
