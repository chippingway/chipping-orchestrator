...
not a weaker binding, it is no binding: the reconciliation proves the repository,
the pull request, the branch, the commit and the requirements revision TOGETHER,
and a group missing one of them would have that proof silently skipped. A
...
place holding somebody else's text.
"""
...
def location_from(recorded: dict) -> ReportLocation | None:
    ...
    A pull request alone is the description, which is a real location. A
    comment id beside it has to be an identity: recorded and unreadable, it is
    a record claiming one place and able to name another, and answering with
    the description would reread a body nobody verified.
    """
    pr_number = _payloads.as_identity(recorded.get(_LOCATION_PR))
    if not pr_number:
        return None
    if _LOCATION_COMMENT not in recorded or recorded[_LOCATION_COMMENT] is None:
        return ReportLocation(pr_number=pr_number)
    comment_id = _payloads.as_identity(recorded[_LOCATION_COMMENT])
    if not comment_id:
        return None
