...
what HTML shows literally or hides, which is `report_html_literals`'s. That one
is handed each span apart rather than their union: spans of two readings
overlap, and made one they would stand an opening tag and its closing tag in
the same code where no reading hides both.
...
`report_locations` is the one reader that asks, and no stage asks it yet.
"""
