...
runs to the end. The same code is the same under EVERY reading -- whatever may
be code holding the one tag holds the other -- so two spans that overlap are
never one stretch of code: one reading's span may hide the opening tag and
another's the closing one, and no reading hides both. An opening tag or comment
opens nothing where it stands in DEFINITE code -- a span or a closed fence that
is code however the text is read -- since a tag quoted as an example is no tag,
and read as one it would take every line after it. Inside an element already
open that mercy ends: no Markdown is read there, so every tag is a tag.
...
def html_literals(
    text: str, code: Iterable[_Stretch], definite: list[_Stretch],
) -> Iterator[_Stretch]:
    """Every stretch of `text` HTML shows literally or hides.

    `code` is every stretch that MAY be code, those of different readings
    overlapping as they will, and `definite` every stretch that certainly is,
    in order and apart.
    """
    reading = _LiteralReading(_PossibleCode(code), _Stretches(definite))
    tags = TagEnds(text)
...
class _PossibleCode:
    """Stretches that may be code, and may overlap, asked which hold a position.

    Dealt into layers each in order and apart -- a stretch to the layer whose
    last one ends first, where it fits behind that one -- so a layer is asked
    with one bisection. There are as many as stretches ever stand over one
    position: no more than the readings one block is given, and the run of
    code lines around them.
    """

    def __init__(self, stretches: Iterable[_Stretch]) -> None:
        layers: list[list[_Stretch]] = []
        last_ends: list[tuple[int, int]] = []
        for stretch in sorted(set(stretches)):
            if last_ends and last_ends[0][0] <= stretch[0]:
                layer = heapq.heappop(last_ends)[1]
            else:
                layer = len(layers)
                layers.append([])
            layers[layer].append(stretch)
            heapq.heappush(last_ends, (stretch[1], layer))
        self._layers: Final = tuple(map(_Stretches, layers))

    def holding(self, position: int) -> tuple[int, ...]:
        """Which stretch of each layer holds `position`, `_OUTSIDE` for none.

        The same answer for two positions is the same stretches holding both,
        a stretch standing in one layer alone.
        """
        return tuple(layer.holding(position) for layer in self._layers)
...
class _LiteralReading:
    ...
    def __init__(self, code: _PossibleCode, definite: _Stretches) -> None:
        self.open_since: int | None = None
        self._code: Final = code
        self._definite: Final = definite
        self._stands_in: tuple[int, ...] = ()
