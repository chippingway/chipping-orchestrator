...
def settles_this_transaction(
    ...
    nothing parked for a human to see. The KIND of place travels with that
    content, since a publication leaves a comment and a verification leaves
    whatever it read.
    """
    if current is None:
        return False
...
def _settles_the_content(
    ...
    not the comment id GitHub will answer the post with, so the digest is what
    tells its report from another. A VERIFY knows BOTH: the exact location it
    ...
    WHICH id a publication landed under is not asked, and that is the mode
    knowing less rather than an oversight: the comment is GitHub's answer to a
    request this transaction had not made when it was recorded, so a record
    carrying any id at all is consistent with it. That there IS one is asked,
    because this road publishes a COMMENT and refuses to settle a post whose
    comment it could not read -- so a record naming the pull request's
    description is not one a publication here ever wrote. Waved through on the
    digest alone it reads as this transaction's completion, and the pending
    record is dropped with its report never published, never posted, and
    nothing parked for anyone to see.
    ...
    return (
        current.location.comment_id is not None
        and current.content_revision == _reports.content_digest(pending.report)
    )
