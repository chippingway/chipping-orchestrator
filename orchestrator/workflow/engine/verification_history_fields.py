...
def historical_from(recorded: object) -> _records.HistoricalEvidence | None:
    ...
    `comment` is `null` exactly for an abandoned transaction: only a
    settlement records the comment an artifact landed as, and only the
    current evidence a settlement installs is ever superseded or invalidated,
    while a pending record never held one. Absent, `null` beside any other
    retirement, a comment beside an abandonment, or present and not an
    identity, it is damage.
    """
    ...
    if members is None or binding is None or retired is None:
        return None
    if (raw_comment is None) != (retired is _records.Retirement.ABANDONED):
        return None
    comment_id = _record_values.as_recorded_number(raw_comment)
