...
record here is: a binding short of any member is no binding, since whoever
relies on it has to prove its members TOGETHER, on a tick with no run behind
it. The review subject in particular has to agree with the binding around it
-- the same pull request and the same requirements revision -- because the
artifact names one requirements revision for both, and a subject about
another pull request is evidence answering for work it never saw.
...
def _target_from(recorded: dict) -> _records.EvidenceTarget | None:
    ...
    The subject has to read whole through its own owner's reader, name a
    developer report -- evidence answers for the report a reviewer was
    handed, and a subject naming none binds it to no report at all -- and be
    about this pull request and these requirements.
    """
    ...
    if identity is None or None in identity:
        return None
    if identity[0] != publication.pr_number:
        return None
