...
workflow/                   publishes labels, transition guards, and the lazy per-repo tick entry point
  ...
  engine/                   what every stage is driven by
    ...
    report_delivery.py      the report one finished run earns, recorded before its code is published: the outcome
                            ...
                            as a question. `owes_a_report` is the one part read live: `dev_pr.py`'s body asks it
                            before writing the run's closing message, and it answers False on every live issue,
                            since no stage CALLS the recording, the park, or `redelivers_an_owed_report` yet
    report_settlement_state.py the current report and the handoff receipt, written in the one durable write that
                            ...
                            and `dev_pr.py`'s body asks `carries_settled_record` before writing the run's closing
                            message
    report_locations.py     which places on a pull request this issue's reports claim, asked of all three records
                            ...
                            it hands back is a claim, for its caller to re-read. Asked by `report_binding.py` and
                            `stages/implementing/pr_description.py`, which no stage CALLS yet
    ...
    report_binding.py       what a publication does with the report its run delivered once the push has landed:
                            ...
                            reconciliation to park. No stage CALLS this yet
  ...
  stages/
    ...
    implementing/           `workflow:implementing`
      ...
      resume_batch.py       the frozen reply batch every `implementing` and `validating` awaiting-human resume
                            ...
                            re-grounding conversations, and command ownership are all cut. The measurement retry
                            and, where the caller says it has already looked (`implementing`'s preflight), the
                            parked-continue classifier are asked of the delivered replies; the authorization
                            command is asked of the last reply the id ledger leaves, and while its park stands it
                            is in no prompt text even where something after it demoted it. An owned batch is handed
                            back whole, delivering and settling nothing. Settlement is the ordinary issue-only
                            `prompt_delivery` ratchet, taken after the run and only for an outcome that counts as
                            delivered: not a live pause, a shutdown kill, or a launch never invoked
      ...
      disposition.py        run-output attribution, inherited floors, timeout parks and their recovery, and agent-result
                            ...
                            and the timeout park is bounded so its notice cannot carry the watermark over a reply
                            written while the agent was out
      ...
      continue_command.py   `/orchestrator continue` on a parked issue, opening with the one park below that the
                            ...
                            `resume_batch.py` froze -- the replies the resume behind it would DELIVER -- so a notice
                            of ours or a forged marker above a bare command cannot turn an explicit retry into a
                            generic resume over prose. The retry is re-grounded, wherever its session is missing or
                            retired, off that freeze's conversation less the commands it consumes
    ...
    validating/             `workflow:validating`
      ...
      dev_fix.py            what a finished dev fix leaves behind: the no-commit reading and the head it carries
                            ...
                            issue object -- the push and the approval it spends, the round bump, and the bounded
                            timeout park
