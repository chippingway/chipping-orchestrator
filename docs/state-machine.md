...
back the same way, on the labels the refresh drives, unless a freeze keeps the refresh away, a stage's own park
stands, or the checkout's HEAD names a commit nothing can read — since none of those is released by the refresh; a
base lag the refresh cannot count over a pinned anchor is reset and parked by the refresh itself. A checkout that is
not on disk stays held, and the dispatcher restores it with nothing run behind the restore, so the next refresh walks
it rather than a handler rebuilding it onto an unpublished replay. Both roads that reconciliation
