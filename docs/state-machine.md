...
stands, or the checkout is one the refresh cannot reach — absent, or with a HEAD naming a commit it cannot read —
since none of those is released by the refresh; a base lag the refresh cannot count over a pinned anchor is reset
and parked by the refresh itself. Both roads that reconciliation
