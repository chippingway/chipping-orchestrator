...
- [`workflow/roles.md`](workflow/roles.md) — the three roles and the stages that spawn each one, where a handler and
  its spawn live, the session-reuse rules per role, the lifetime agent-run ledger every role spends from and the
  counters that reset without returning a run, the size gate every clean committed candidate passes before it is
  published and what its number earns — the exact commit an authorized settlement exempts included, and the proven
  workflow rewrites that exemption may rotate onto — what the late adjudication of an oversized committed candidate is
...
run wrote before the size gate and the push, and `workflow/engine/report_binding.py` binds it to the pull request the
code reaches and publishes it there before the issue may move to `workflow:validating`. The additive
`developer_report_*` pinned records (`workflow/engine/report_record*`, `report_delivery_state.py` and
`report_settlement_state.py`) carry that report and the publication transaction it is bound into across a process
that dies mid-way; `workflow/engine/report_transaction.py` reconciles an outstanding transaction ahead of every stage
handler — proving the world it was recorded against, publishing or re-reading the report, and settling the record in
a single write once the issue's requirements read unchanged after that request, on this road and the binding's
alike. Full contract:
