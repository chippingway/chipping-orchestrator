...
class LayeringTest(unittest.TestCase):
    ...
    def test_initializer_reaches_no_owner(self) -> None:
        # An eager owner binding here would charge a counter-key read for the
...
class PackageSurfaceTest(unittest.TestCase):
    ...
    def test_package_sits_under_the_stage_package(self) -> None:
        ...
        self.assertEqual(initializer.parent.name, "conflicts")
        self.assertIs(importlib.import_module(_PARENT).conflicts, _package)
...
class DispatchTargetTest(unittest.TestCase):
    ...
    def test_label_resolves_to_the_handler_owner(self) -> None:
        ...
        self.assertEqual(
            _stage_targets._STAGE_HANDLER_TARGETS[WorkflowLabel.RESOLVING_CONFLICT],
            (owner.__name__, _HANDLE_RESOLVING_CONFLICT),
        )
        self.assertTrue(hasattr(owner, _HANDLE_RESOLVING_CONFLICT))
