...
class AmbiguousReceiptTest(OrphanAdoptionCase, unittest.TestCase):
    """An issue carrying two slices' receipts is adopted for neither.

    The lookup matches a marker as a substring of a body that carries declared
    scope above it, so an issue whose body holds a second slice's receipt
    answers that slice's search as readily as its own. Adopted on the match
    alone, one issue is recorded as two children of the same split -- the
    parent's register naming it twice, the consumer ledger naming it once, and
    its ancestry overwritten with the other slice's scope.
    """
    ...
    def test_a_second_slice_is_not_adopted_by_receipt(self) -> None:
        ...
        self.assertEqual(outcome.disposition, _LateDisposition.PARKED)
        self.assertEqual(len(self.github.created_child_issues), 1)
        self.assertEqual(self._recorded(), [first.number])
        self.assertEqual(self._pinned()[KEY_CONSUMERS], [first.number])
