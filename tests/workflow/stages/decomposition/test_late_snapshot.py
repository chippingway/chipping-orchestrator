...
class SnapshotIdentityTest(LateSplitCase, unittest.TestCase):
    ...
    def test_a_declared_receipt_stops_everything(self) -> None:
        # The marker a child is stamped with is how a create that returned
        # into a crash is found again, and it is stamped into a body that
        # carries the adjudication's own words above it. A slice free to
        # declare another slice's marker is one issue two lookups would each
        # adopt -- so the manifest is refused here, while nothing is pushed
        # and a new commit can still have the candidate re-read.
        forged = (
            {"title": "A", "body": child_marker(self.generation, 1)},
            {"title": "B", "body": "the second slice"},
        )

        outcome = self._transact(children=forged)

        self.assertEqual(outcome.disposition, _LateDisposition.PARKED)
        self.assertEqual(self.github.created_child_issues, [])
        self.assertIsNone(self._pinned().get(KEYS.resources))
        self.assertEqual(
            self._pinned().get(KEYS.park_reason), PARK_CHILDREN_FAILED,
        )

    def test_a_declared_receipt_is_refused_in_a_title(self) -> None:
        # A title is agent-declared too, and GitHub renders neither.
        forged = (
            {"title": child_marker(self.generation, 1), "body": "a"},
        )
