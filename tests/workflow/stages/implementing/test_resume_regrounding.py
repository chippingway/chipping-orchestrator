...
_MISSING_SESSION = MappingProxyType({
    _state._DEV_AGENT: _retry_support.BACKEND_CLAUDE,
})
_RETRIED_SESSIONS = (
    ("a missing session", _MISSING_SESSION),
    ("a retired session", _RETIRED_SESSION),
)

# Each of those, with what the park had already consumed on the thread -- and
# a thread with nothing on it but the command the retry consumes, whose frozen
# conversation is EMPTY. That is still the caller's answer: read again at
# spawn time instead, the prompt would quote the command and the reply
# written while it is built, neither of which the retry settles.
_RETRIED_THREADS = (
    *((described, session, _SAID_BEFORE) for described, session in _RETRIED_SESSIONS),
    ("nothing but the command", _MISSING_SESSION, ""),
)
...
class FreshSpawnRegroundingTest(_support._ParkedThread, unittest.TestCase):
    ...
    def test_a_retry_quotes_the_thread_it_classified(self) -> None:
        ...
        # said.
        for described, session, settled in _RETRIED_THREADS:
            with self.subTest(thread=described):
                self.setUp()

                prompt = self._retried_over(session, settled)
    ...
    def _retried_over(self, session, settled: str = _SAID_BEFORE) -> str:
        """The retry's own prompt, on a timeout park with `session` pinned.

        A reply the park already consumed, where there is one, stays in the
        conversation, and one lands at the session read -- the window between
        the freeze and the prompt -- while the retry is being built.
        """
        pinned = {_state._PARK_REASON: _state._AGENT_TIMEOUT, **session}
        if settled:
            pinned[_state._LAST_ACTION_COMMENT_ID] = self._they_say(settled)
        self._seed(**pinned)
