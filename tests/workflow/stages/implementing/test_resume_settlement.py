...
from tests.workflow.stages.implementing import (
    ...
    report_test_support as _report_support,
...
# What a run that produced work comes back saying -- ending on its report, as
# the stage holds a finished run to -- and the two heads that say it really
# did: a park past the publication seam is only reached by a commit.
_COMMITTED = _reported()
_COMMITTED_UNREPORTED = "implemented"
...
# A few of `resume_batch`'s ownership cases, asked of the resume: the park,
# the words on its thread, whether the parked-continue classifier has already
# looked, and the index of the reply the resume settles to -- None where
# nothing resumes, because a command road owns the batch or nothing in it is
# the developer's.
_OWNED = (
    ("an authorization command", _late_command.PARK_UNAUTHORIZED_EXEMPTION, (
        _consent_payloads.AUTHORIZE,
    ), False, None),
    ("guidance over one", _late_command.PARK_UNAUTHORIZED_EXEMPTION, (
        _consent_payloads.AUTHORIZE, _support.GUIDANCE,
    ), False, 1),
    ("a forged marker over one", _late_command.PARK_UNAUTHORIZED_EXEMPTION, (
        _consent_payloads.AUTHORIZE, _support.FORGED,
    ), False, None),
    ("a measurement retry", _late_measurement_state.PARK_MEASUREMENT_FAILED, (
        _consent_payloads.CONTINUE,
    ), False, None),
    ("a claimed retry", _state._AGENT_TIMEOUT, (_consent_payloads.CONTINUE,), True, None),
    ("a retry nobody has classified", _state._AGENT_TIMEOUT, (
        _consent_payloads.CONTINUE,
    ), False, 0),
)

# The two reports a completed, committing run can hand over that the workflow
# cannot deliver: none at all, and one past what the pinned comment carries.
_UNDELIVERABLE_REPORTS = (
    ("no report", _COMMITTED_UNREPORTED),
    ("an unrecordable report", _report_support.ready_message(
...
class ResumeSettlementTest(_support._ParkedThread, unittest.TestCase):
    ...
    def test_a_command_road_owns_the_whole_batch(self) -> None:
        # Which batch a command road owns is `resume_batch`'s table; what this
        # road owes an owned one is to resume nothing and consume nothing,
        # since its own run's park would stamp the thread read past the
        # command. Guidance beside the command is an ordinary resume instead,
        # and a demoted authorization command reaches no developer as prose.
        for described, reason, thread, claimed, delivered in _OWNED:
            with self.subTest(thread=described):
                self.setUp()
                self._seed(**{_state._PARK_REASON: reason})
                self._assert_owned(thread, claimed, delivered)

    def _assert_owned(self, thread: tuple, claimed: bool, delivered) -> None:
        spoke = [self._they_say(said) for said in thread]

        resumed = self._resumes(continue_claimed=claimed)
        ...
        if delivered is None:
            ...
            return
        resumed.call.assert_called_once()
        self.assertEqual(self._watermark(), spoke[delivered])
        for withheld in (_support.FORGED, _consent_payloads.AUTHORIZE):
            self.assertNotIn(withheld, resumed.followup)
...
class ContinueReservationTest(_support._ParkedThread, unittest.TestCase):
    ...
    Both the preflight and this reservation read the replies a prompt would
    be built from, so a comment neither would deliver cannot make one of them
    call the batch mixed while the other hands the command over as prose.
    """
...
class RunLimitAuthorizationTest(_consent_case._ParkedCase, unittest.TestCase):
    ...
    def _tick(self):
        ...
        park is about -- over the ceiling, so it is held for an operator. The
        committed candidate's report is re-stamped at the requirements the
        thread now reads at, as `_run_tick` does: the replies these cases post
        move them, and a report naming an older revision is held.
        """
...
class CommittedRunParkTest(_support._ParkedThread, unittest.TestCase):
    ...
    work is committed and the failure is a push, a reading, or a report the
    workflow cannot deliver rather than anything the agent asked. The park is
    still the thing that writes down how far the thread was read, and the run
    it ends is still minutes long.
    """
    ...
    def test_a_report_it_cannot_deliver_keeps_it(self) -> None:
        # The run completed and committed, but handed over no report, or one
        # the pinned comment cannot carry. The report's own park holds the
        # work before anything is published -- and its notice lands above the
        # reply written during the run just the same.
        for described, answered in _UNDELIVERABLE_REPORTS:
            with self.subTest(report=described):
                self.setUp()
                landed = self._commits_over_guidance(
                    answers=_agent(last_message=answered),
                )

                self._assert_kept(landed)
                self.assertEqual(
                    self.github.pinned_data(_support.ISSUE_NUMBER)[_state._PARK_REASON],
    ...
    def _commits_over_guidance(self, answers=None, **run_options) -> tuple:
        """One whole tick: guidance resumes a developer, and it commits.

        It answers with `answers`, a report ready for publication by default.
        """
        self._seed(**{
            _state._DEV_AGENT: _BACKEND,
            _state._DEV_SESSION_ID: _SESSION,
        ...
        spoke = self._they_say(_support.GUIDANCE)
        landing = _RunsWhileOneLands(self, answers=answers)
        self._run_implementing(
            self.github,
            self.issue,
            run_agent=MagicMock(side_effect=landing),
