...
_PR_BODY_AGENT_MESSAGE_CAP = support._PR_BODY_AGENT_MESSAGE_CAP
_PR_BODY_TRUNCATION_MARKER = support._PR_BODY_TRUNCATION_MARKER
_PatchedWorkflowMixin = support._PatchedWorkflowMixin
_agent = support._agent
_format_pr_agent_message = support._format_pr_agent_message
...
class FormatPrAgentMessageTest(unittest.TestCase):
    ...
    def test_short_message_returned_verbatim(self) -> None:
        ...
        out = _format_pr_agent_message(msg)
        self.assertEqual(out, msg)
        self.assertNotIn(_PR_BODY_TRUNCATION_MARKER, out)

    def test_message_at_cap_is_not_marked(self) -> None:
        msg = "x" * _PR_BODY_AGENT_MESSAGE_CAP
        out = _format_pr_agent_message(msg)
        self.assertEqual(out, msg)
        self.assertNotIn(_PR_BODY_TRUNCATION_MARKER, out)
    ...
    def test_long_message_capped_with_marker(self) -> None:
        ...
        self.assertIn(_PR_BODY_TRUNCATION_MARKER, out)
        # The kept text stays within the cap (plus the appended marker).
        self.assertLessEqual(
            len(out),
            _PR_BODY_AGENT_MESSAGE_CAP + len(_PR_BODY_TRUNCATION_MARKER) + 8,
        )
        # And the body ends with the marker, not mid-word.
        self.assertTrue(out.rstrip().endswith(_PR_BODY_TRUNCATION_MARKER))
    ...
    def test_cap_lands_on_token_boundary(self) -> None:
        ...
        head = "a" * (_PR_BODY_AGENT_MESSAGE_CAP - 5)
        tail = "b" * TOKEN_TAIL_LENGTH
        msg = f"{head} {tail}"
        out = _format_pr_agent_message(msg)
        self._kept = out.split(f"\n\n{_PR_BODY_TRUNCATION_MARKER}")[0]
...
class OnCommitsBodyTruncationTest(unittest.TestCase, _PatchedWorkflowMixin):
    ...
    def test_long_body_is_capped_and_marked(self) -> None:
        ...
        self.assertIn(_PR_BODY_TRUNCATION_MARKER, body)
    ...
    def test_short_agent_message_body_no_marker(self) -> None:
        ...
        self.assertNotIn(_PR_BODY_TRUNCATION_MARKER, body)
