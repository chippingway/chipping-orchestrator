...
class _ParkedThread(_PatchedWorkflowMixin):
    ...
    def _seed(self, **pinned) -> None:
        ...
        With the requirements baseline a real park carries -- the thread as
        that park had read it -- so a reply past its watermark moves the hash
        the way it does in production, and the drift check is asked about it.
        """
        floor = pinned.get(_state._LAST_ACTION_COMMENT_ID, PARKED_AT)
        self.github.seed_state(ISSUE_NUMBER, **{
            ...
            _delivery.PINNED_USER_CONTENT_HASH: _content_hash._compute_user_content_hash(
                self.issue, set(), comments=[
                    seen for seen in self.issue.comments if seen.id <= floor
                ],
            ),
            **pinned,
        })
