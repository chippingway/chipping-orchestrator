...
CLOSING_NOTHING = (
    ...
    f'[link](https://example.com "{RESOLVES}")',
    f'[link](https://example.com/a_(b_(c)) "{RESOLVES}")',
    f"![nested [label] {RESOLVES}](https://example.com/a.png)",
    f"![{RESOLVES}][]\n\n[{RESOLVES}]: https://example.com/a.png",
    f"![{RESOLVES}]\n\n[{RESOLVES}]: https://example.com/a.png",
    f'[link](url "first\\\n{RESOLVES}")',
    f'[docs]: url "first\\\n{RESOLVES}"',
    f'> [link](url\n> "{RESOLVES}")',
...
class DescriptionVerdictTest(unittest.TestCase, _ReusedPullRequest):
    ...
    def test_an_unreadable_description_holds(self) -> None:
        # Nobody is asked anything over a description nobody could read, and
        # the next tick reads it again: the lookup refused, or the lookup
        # answered with the lazy pull request PyGithub hands back and the body
        # read behind it is the request that failed.
        for refused in (GET_PR, BODY):
            with self.subTest(refused=refused):
                self.reused()
                unread = {"side_effect": RuntimeError(UNREAD)}
                if refused == BODY:
                    unread = {
                        "return_value": support.LazyPullRequest(self.live, failing=BODY),
                    }

                with patch.object(self.github, GET_PR, **unread):
                    verdict = self.names()
