...
"""A push that landed and a report that did not, and the tick that finishes it.

The window is the one every publication has: the branch is on the remote, a
pull request carries it, and the comment the report goes in is a separate
request that GitHub can refuse or accept without saying so. What the stage owes
there is to keep the work exactly where it is -- unhanded-on, unparked, and
recorded as still owing a report -- so the next poll finishes it with no
developer run, no second pull request, and no second report.

The retry runs over the world the first tick left, which is the world the size
gate calls DELIVERED: the receipt names the commit, and the pull request is
standing on it, so the push moves nothing and the bookkeeping is all that is
left.
"""
...
from tests.workflow.fixtures import LABEL_IMPLEMENTING, LABEL_VALIDATING
...
class ReportDebtTest(unittest.TestCase, support._ReportDeliveryMixin):
    ...
    def test_an_unrecordable_report_holds_nothing(self) -> None:
        # A report past what the pinned comment can carry is one no later tick
        # could publish either, so the code goes out, the reviewer is handed
        # no report, and the issue is not stranded on a text.
        github, issue = self.seeded()
        ...
        self.assertNotIn(support.PENDING_RECORD, recorded)
        self.assertIn(
            (support.REPORT_ISSUE, LABEL_VALIDATING), github.label_history,
        )
        self.assertNotIn(
            (support.REPORT_ISSUE, LABEL_IMPLEMENTING), github.label_history,
        )
