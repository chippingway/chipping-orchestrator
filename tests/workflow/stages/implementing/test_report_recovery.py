...
# What a human adds under the pull request's description while its report is
# still owed, which the retry has to leave exactly where they put it.
HUMAN_NOTE = "A note added by hand while the report was owed."
...
class ReportDebtTest(unittest.TestCase, support._ReportDeliveryMixin):
    ...
    def _retry_settles(self, github, issue) -> None:
        """Run the next poll, a human having edited the description, and prove it settled."""
        ...
        self.assertEqual(
            ...
            (1, 1, None, None, OPENED_PR, [], edited),
        )
        self.assertIn(
            (support.REPORT_ISSUE, LABEL_VALIDATING), github.label_history,
        )
