...
class UndeliverableReportTest(unittest.TestCase, support._ReportDeliveryMixin):
    ...
    def test_a_description_a_human_names_is_handed_on(self) -> None:
        # Held, the body is untouched. The human puts the two lines the notice
        # quotes above their own words and replies; the reply's report goes in
        # a COMMENT, and the description stands exactly as they left it.
        github, issue, reused = self._collided_on_the_description()
        self.assertEqual(
            (github.edited_pr_bodies, github.label_history), ([], []),
        )
        named = "\n\n".join((
            _named_description(support.REPORT_ISSUE, support.DEV_SESSION),
            HUMAN_DESCRIPTION,
        ))
        reused.body = named
        support.replies(github, issue, "named it; put the report in a comment")
