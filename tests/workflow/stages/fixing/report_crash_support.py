...
push, so every window past that write is one a tick can die in and come back
to an issue that still says what its developer reported. These are the shapes
those windows leave -- a delivery nothing bound, with or without a receipt
beside it -- and the reading a case about such a window actually needs: not
what the comment says once the dust settles, but what the FIRST write that
mattered already said.
"""
