...
class CleanProcessImportTest(unittest.TestCase):
    ...
    engine's dispatcher reaches back into this package. A recording per module
    ...
    layering check below reads the package's planted set off the same
    recording, so it costs no interpreter of its own.
    """
...
class DispatchTargetTest(unittest.TestCase):
    ...
    def test_label_resolves_to_the_handler_owner(self) -> None:
        ...
        self.assertEqual(
            _stage_targets._STAGE_HANDLER_TARGETS[WorkflowLabel.QUESTION],
