...
_ATTRIBUTION = (
    f"Generated by orchestrator ({_support.SPEC_BACKEND} session "
    f"`{_support.DISCUSSION_SESSION}`) in the `discussion` stage."
)
...
class DiscussionPlanPrReuseTest(unittest.TestCase, _DiscussionWorkflowMixin):
    ...
    def test_an_open_pr_is_adopted_not_duplicated(self) -> None:
        ...
        self.assertEqual(reused.pinned[_support.KEY_PR_NUMBER], _EXISTING_PR_NUMBER)
        self.assertEqual(
            reused.pinned[KEY_PARK_REASON], _support.PARK_DISCUSSION_PLAN_PUBLISHED,
        )
...
class DiscussionMergedPlanPrTest(unittest.TestCase, _DiscussionWorkflowMixin):
    ...
    def test_a_closed_pr_is_recorded_not_replaced(self) -> None:
        ...
        self.assertEqual(
            (
                ...
                pinned_data[_support.KEY_ROUND_OPEN],
            ),
    ...
    def test_an_amended_merged_pr_is_recorded(self) -> None:
        ...
        self.assertEqual(pinned_data[_support.KEY_PR_NUMBER], _AMENDED_PR_NUMBER)
    ...
    def test_an_unreadable_lookup_holds_publishing(self) -> None:
        ...
        held[_support.PUSH_BRANCH].assert_not_called()
        self.assertEqual((gh.opened_prs, gh.posted_comments), ([], []))
        pinned_data = gh.pinned_data(held_issue.number)
        # The marker stands, which is what carries the retry to the next tick.
        self.assertEqual(
            (pinned_data.get(_support.KEY_PR_NUMBER), pinned_data[_support.KEY_PUBLISHING_SHA]),
            (None, _support.HEAD_AFTER_COMMIT),
        )
        ...
        pinned_data = gh.pinned_data(held_issue.number)
        self.assertEqual(
            (pinned_data[_support.KEY_PR_NUMBER], pinned_data[_support.KEY_PUBLISHING_SHA]),
    ...
    def test_a_sessionless_adoption_is_refused(self) -> None:
        ...
        for issue_number, merged in _SESSIONLESS_ADOPTIONS:
            with self.subTest(merged=merged):
                ...
                self.assertEqual(
                    (
                        pinned_data.get(_support.KEY_PR_NUMBER),
                        pinned_data[KEY_PARK_REASON],
                    ),
                    (None, _support.PARK_DISCUSSION_UNATTRIBUTED),
                )
    ...
    *,
    ...
    gh.seed_state(
        ...
        **{
            ...
            _support.KEY_ROUND_BRANCH: branch,
            _support.KEY_ROUND_SHA: _support.HEAD_BEFORE_ROUND,
            _support.KEY_ROUND_OPEN: round_open or None,
            _support.KEY_BASE_SHA: BASE_TIP_SHA,
            _support.KEY_DISCUSSION_AGENT: config.DECOMPOSE_AGENT_SPEC,
            _support.KEY_DISCUSSION_SESSION_ID: _support.DISCUSSION_SESSION if session else None,
        },
    )
    gh.add_pr(FakePR(
...
def _seed_amended_publication(
    ...
    gh.add_pr(FakePR(
        ...
        head_branch=_support._issue_branch(issue_number),
