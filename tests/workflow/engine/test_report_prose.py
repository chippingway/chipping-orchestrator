...
CODE_IN_HTML = (
    ...
    # Nor one hidden by a span of another reading than the opening tag's: the
    # two spans overlap, yet no reading hides both tags.
    f"https://example.com/`x <pre>y`</pre>` {FIXES}",
    f"<pre><!-- </pre> --> {FIXES}</pre>",
...
PROSE = (
    ...
    f"| a | b |\n| --- | --- |\n| `x` | `y` |\n\n{FIXES}",
    f"```\n<pre\n```\n\n{FIXES}",
    # An element one span holds whole is closed under every reading, whatever
    # span of another reading overlaps it.
    f"https://example.com/`x `<pre>y</pre>` {FIXES}",
