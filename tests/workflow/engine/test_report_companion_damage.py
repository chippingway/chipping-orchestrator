...
# The pull request's own body, which is what a location naming no comment is.
_DESCRIPTION = ReportLocation(pr_number=support.PR_NUMBER)
...
class SettledLocationKindTest(unittest.TestCase, support.ReportTransactionCase):
    """A settled record at the pull request's DESCRIPTION belongs to one mode.

    `null` in a location's comment field is the description, and that is a real
    place: a verification reads the pull request's own body there and records
    exactly what it read. A publication never does -- it posts a COMMENT, and
    refuses to settle a post whose comment it could not read.

    So one settled shape is a transaction's own completion under one mode and a
    record nothing here wrote under the other, and everything the pending
    record is for hangs on telling them apart. The pull request's body is the
    report text the publication carries, so the two cases differ in the LOCATION
    alone: the subject, the revision, the digest and the handoff all agree, and
    a publication accepted on them is dropped with its report never published,
    never posted, and nothing parked for anyone to see.
    """

    def setUp(self) -> None:
        support.ReportTransactionCase.setUp(self)
        self.pull_request.body = support.REPORT_TEXT
        self._settled_at_the_description()

    def test_a_publication_is_not_settled_by_one(self) -> None:
        self.record()
    ...
    def test_a_verification_at_one_replays(self) -> None:
        # The control the refusal above is read against: the same settled
        # shape, under the mode that does record it, is that transaction's own
        # completion -- recognized, dropped, and nothing posted beside it.
        self.record(
            mode=_records.ReportMode.VERIFY,
            report="",
    ...
    def _settled_at_the_description(self) -> None:
        """A settlement recorded as sitting on the pull request's own body."""
        pending = self.pending()
        fixture.records_current(
            self.state, pending.subject, pending.report_revision,
            location=_DESCRIPTION,
...
class ForeignParkTest(unittest.TestCase, support.ReportTransactionCase):
    """A park another route took is never replaced, and never blocked."""
    ...
    def test_a_foreign_park_stands_the_tick_down(self) -> None:
        # What answers a foreign park is the stage handler BEHIND this guard.
        # Held in front of it that branch never runs, so the park never
        # clears, so `awaiting_human` stays set and this holds again -- both
        # parks standing for the life of the issue with neither the damage nor
        # the recovery ever announced.
        self.assertFalse(self.reconcile())

    def test_the_damage_follows_that_park_down(self) -> None:
        # The other half of standing down: the damage is answered on the tick
        # after the foreign park goes, rather than never.
        self.reconcile()
        self.state.set(support.AWAITING_HUMAN, False)
        self.state.set(support.PARK_REASON, None)
