...
class EndedPullRequestTest(unittest.TestCase, support.ReportTransactionCase):
    ...
    def test_a_sound_transaction_retires_on_an_ending(self) -> None:
        # The plain retirement the damaged ones below are variations on. A
        # report posted onto a merged or closed thread is a comment nobody
        # reads, so the transaction is dropped rather than held for work that
        # is over.
