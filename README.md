...
3. `workflow:validating` — a fresh reviewer checks the diff. Requested changes enter `workflow:fixing` and return
   ...
   open resumes the dev agent here; the report it writes is posted on the pull request — with its fix, or on its own
   where no code had to change — and no reviewer runs until the pull request carries it.
